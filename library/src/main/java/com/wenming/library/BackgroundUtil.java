...
//                return getApplicationValue((MyApplication) ((Service) context).getApplication());
     ...
     * @return
     */
    public static boolean isForegroundBoolean(Context context, int methodID, String packageName) {
        switch (methodID) {
            case BKGMETHOD_GETRUNNING_TASK:
                return getRunningTaskBoolean(context, packageName);
            case BKGMETHOD_GETRUNNING_PROCESS:
                return getRunningAppProcessesBoolean(context, packageName);
            case BKGMETHOD_GETAPPLICATION_VALUE:
                return getApplicationValueBoolean((MyApplication) ((Service) context).getApplication());
            case BKGMETHOD_GETUSAGESTATS:
                return queryUsageStatsBoolean(context, packageName);
            case BKGMETHOD_GETACCESSIBILITYSERVICE:
                return getFromAccessibilityServiceBoolean(context, packageName);
            case BKGMETHOD_GETLINUXPROCESS:
                return getLinuxCoreInfoBoolean(context, packageName);
            default:
                return false;
        }
    }
     ...
     * @return
     */
    public static String getRunningTask(Context context, String packageName) {
     ...
     * @return
     */
    public static boolean getRunningTaskBoolean(Context context, String packageName) {
        ActivityManager am = (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE);
        ComponentName cn = am.getRunningTasks(1).get(0).topActivity;
     ...
     * @return
     */
    public static String getRunningAppProcesses(Context context, String packageName) {
     ...
     * @return
     */
    public static boolean getRunningAppProcessesBoolean(Context context, String packageName) {
        ActivityManager activityManager = (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE);
        List<ActivityManager.RunningAppProcessInfo> appProcesses = activityManager.getRunningAppProcesses();
        if (appProcesses == null) {
            return false;
    ...
    public static boolean getApplicationValueBoolean(MyApplication myApplication) {
        return myApplication.getAppCount() > 0;
    }
...
//        }
    ...
    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public static boolean queryUsageStatsBoolean(Context context, String packageName) {
        class RecentUseComparator implements Comparator<UsageStats> {
            @Override
            public int compare(UsageStats lhs, UsageStats rhs) {
                return (lhs.getLastTimeUsed() > rhs.getLastTimeUsed()) ? -1 : (lhs.getLastTimeUsed() == rhs.getLastTimeUsed()) ? 0 : 1;
     ...
     * @return
     */
    public static String getFromAccessibilityService(Context context, String packageName) {
        if (DetectService.isAccessibilitySettingsOn(context) == true) {
            ...
            Log.d("wenming", "**方法五**0 当前窗口焦点对应的包名为： =" + foreground);
//            if (packageName.equals(foreground)){
...
//            }
     ...
     * @return
     */
    public static boolean getFromAccessibilityServiceBoolean(Context context, String packageName) {
        if (DetectService.isAccessibilitySettingsOn(context) == true) {
            DetectService detectService = DetectService.getInstance();
            String foreground = detectService.getForegroundPackage();
            SharePreferences(context,foreground);
     ...
     * @param packageName 需要检查是否位于栈顶的App的包名
     */
    public static String getLinuxCoreInfo(Context context, String packageName) {
     ...
     * @param packageName 需要检查是否位于栈顶的App的包名
     */
    public static boolean getLinuxCoreInfoBoolean(Context context, String packageName) {
